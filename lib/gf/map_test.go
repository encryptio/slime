...
func TestMapTrivial(t *testing.T) {
	tests := []struct {
		...
		{[]byte{0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 0, []uint32{0xFF000000, 0x00000000}},
		{[]byte{0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 0, []uint32{0xFFFF0000, 0x00000000}},
		{[]byte{0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00}, 0, []uint32{0xFFFFFF00, 0x00000000}},
		{[]byte{0x0F, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00, 0x00}, 0, []uint32{0x0FFFFFFF, 0x00000000}},
		{[]byte{0x0F, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00, 0x00}, 0, []uint32{0x0FFFFFFF, 0xFF000000}},
		{[]byte{0x0F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00, 0x00}, 0, []uint32{0x0FFFFFFF, 0xFFFF0000}},
		{[]byte{0x0F, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x00}, 0, []uint32{0x0FFFFFFF, 0xFFFFFF00}},
		{[]byte{0x0F, 0xFF, 0xFF, 0xFF, 0x0F, 0xFF, 0xFF, 0xFF}, 0, []uint32{0x0FFFFFFF, 0x0FFFFFFF}},
	...
	for _, test := range tests {
		...
		if !bytes.Equal(test.In, remapped) {
			t.Errorf("MapFromGF(%v,%v) = %v, wanted %v", gotn, gotv, remapped, test.In)
		}
	}
}
...
func TestMapTricky(t *testing.T) {
	...
	for _, test := range tests {
		...
		for i := 0; i < len(gotv); i++ {
			if gotv[i] >= MaxVal {
				t.Errorf("MapToGF(%v) = %v,%v, but element %v is %v, which is over %v", test, gotn, gotv, i, gotv[i], MaxVal)
				continue
			}
		}
		...
		if !bytes.Equal(test, remapped) {
			t.Errorf("MapFromGF(%v,%v) = %v, wanted %v", gotn, gotv, remapped, test)
		}
	}
}
...
func BenchmarkMapToGF64kWorst(b *testing.B) {
	...
	copy(data[len(data)-8:], []byte{0xFF, 0xFF, 0xFF, 0xFF, 0x7F, 0xFF, 0xFF, 0xFF})
